...
namespace mocr
{
	...
	private:
		Model(std::vector<Tensor<double>> &&data, std::vector<Tensor<double>> &&gradient, std::vector<std::shared_ptr<Layer<double>>> &&layers) : data_(std::move(data)),
	...
	public:
		Builder(int inputChannels, int inputWidth, int inputHeight) : layerCount_(0), inpLayer_{inputChannels, inputWidth, inputHeight}
		{
		}
